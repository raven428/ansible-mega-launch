...
class ActionModule(ActionBase):
  ...
  def run(self, tmp=None, task_vars=None):
    ...
    self._display.warning('test warning from plugin')
    ...
    results = merge_hash(
      ...
      self._execute_module(
        module_name='mega_status', task_vars=task_vars, module_args=new_module_args
      )
    )

    for line in list(results['warning_lines'] if 'warning_lines' in results else []):
